...
def authentication_token_from_email(
    *, api_client: TestClient, email: str, db: Database
    ...
    if not user:
        first_name = random_lower_string()
        last_name = random_lower_string()
        user_in_create = UserIn(email=email, password=password, first_name=first_name, last_name=last_name)
        await crud.user.create(db, obj_in=user_in_create)
    else:
        user_in_update = UserUpdate(password=password)
        await crud.user.update(db, db_obj=user, obj_in=user_in_update)

    return user_authentication_headers(api_client=api_client, email=email, password=password)
